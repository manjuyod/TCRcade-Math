...
export class DatabaseStorage implements IStorage {
  ...
  /**
   * Get all friends for a user (accepted status only)
   */
  async getUserFriends(userId: number): Promise<Array<User & { friendshipId: number }>> {
    // Find all accepted friendship relations where the user is either the sender or receiver
    ...
    // Map friendship IDs to each user
  ...
  async getPendingFriendRequests(userId: number): Promise<Array<User & { requestId: number }>> {
    ...
    // Get the IDs of users who sent requests
    const requestorIds = pendingRequests.map(r => r.userId);
    ...
    // Get user details for all requestors
    const requestUsers = await db
      .select()
      .from(users)
      .where(inArray(users.id, requestorIds));

    // Map request IDs to each user
  ...
  async getSentFriendRequests(userId: number): Promise<Array<User & { requestId: number }>> {
    // Find all pending friend requests where the user is the sender
    ...
    // Get the IDs of users who received requests
    const receiverIds = sentRequests.map(r => r.friendId);

    // If no sent requests, return empty array
    if (receiverIds.length === 0) {
      return [];
    }

    // Get user details for all receivers
    ...
    // Map request IDs to each user
    return receiverUsers.map(user => {
      const request = sentRequests.find(r => r.friendId === user.id);
   ...
   */
  async sendFriendRequest(userId: number, friendId: number): Promise<{ success: boolean; message: string; request?: Friend }> {
    ...
    if (existingRelation) {
      ...
      if (existingRelation.status === "pending") {
        if (existingRelation.userId === userId) {
          ...
          // The other user sent a request to this user, so we can accept it
          ...
          return { success: true, message: "Friend request accepted", request: updatedRelation };
        }
      }
    ...
    return { success: true, message: "Friend request sent", request: newRequest };
  }

  /**
   * Respond to a friend request (accept or reject)
   */
  async respondToFriendRequest(requestId: number, userId: number, accept: boolean): Promise<{ success: boolean; message: string }> {
    ...
    if (request.friendId !== userId) {
      return { success: false, message: "You do not have permission to respond to this request" };
    }
   ...
   */
  async removeFriend(userId: number, friendshipId: number): Promise<{ success: boolean; message: string }> {
    ...
    if (friendship.userId !== userId && friendship.friendId !== userId) {
      return { success: false, message: "You do not have permission to remove this friendship" };
    }
  ...
  async searchUsers(query: string, currentUserId: number): Promise<User[]> {
    ...
    // Search for users with similar usernames, excluding the current user
