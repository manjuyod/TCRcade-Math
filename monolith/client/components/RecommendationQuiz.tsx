...
import { CheckCircle, XCircle, Brain, Target, Clock, Star } from 'lucide-react';
...
export default function RecommendationQuiz() {
  ...
  const { data: recommendationData, isLoading: isLoadingRecommendations } = useQuery({
    queryKey: ['/api/monolith/recommendations'],
    queryFn: () => apiRequest('GET', '/api/monolith/recommendations?maxQuestions=10&sessionType=practice'),
  });

  // Fetch questions based on recommendations
  const { data: questions, isLoading: isLoadingQuestions } = useQuery({
    queryKey: ['/api/questions/batch', recommendationData?.recommendations],
    queryFn: async () => {
      if (!recommendationData?.recommendations) return [];
      
      const questionIds = recommendationData.recommendations.map(r => r.questionId);
      const questions = await Promise.all(
        questionIds.map(id => apiRequest('GET', `/api/questions/${id}`))
      );
      return questions.filter(q => q !== null);
    },
    enabled: !!recommendationData?.recommendations
  });
  ...
  const handleNextQuestion = () => {
    if (!currentQuestion || !selectedAnswer || !currentRecommendation) return;
    ...
    feedbackMutation.mutate({
      ...
      engagementRating: timeSpent > 5 && timeSpent < 120 ? 5 : 3 // Simple engagement metric
    });
  ...
  if (isLoadingRecommendations || isLoadingQuestions) {
    return (
