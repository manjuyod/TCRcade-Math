...
export default function DivisionPlayPage() {
  ...
  const loadPreloadedQuestions = () => {
    try {
      ...
      if (questionsData && operation === 'division') {
        ...
        if (questions.length > 0) {
          ...
          setIsLoading(false);
          console.log(`Loaded ${questions.length} pre-loaded division questions`);
        } else {
  ...
  const handleAnswer = async (selectedAnswer: string) => {
    ...
    if (shouldComplete) {
      try {
        ...
        setLocation(`/math-facts/division/complete?${params.toString()}`);
      } catch (error) {
