...
export default function FractionsPlayPage() {
  ...
  useEffect(() => {
    ...
    correctAudio.src =
      "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhEAAAFWq79uJbGggAAA==";
    correctSoundRef.current = correctAudio;

    // Create error sound (lower tone)
    const incorrectAudio = new Audio();
    incorrectAudio.src =
      "data:audio/wav;base64,UklGRnoGAABXQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YQoGAACBhYqFbF1fdJivrJBhNjVgodDbq2EcBj+a2/LDciUFLIHO8tiJNwgZaLvt559NEAxQp+PwtmMcBjiR1/LMeSwFJHfH8N2QQAoUXrTp66hVFApGn+DyvmwhEAAAF0q39uFZGAcAAA==";
  ...
  const progress =
    ((currentQuestionIndex + 1) / FRACTIONS_PUZZLE_RULES.questionCount) * 100;
  ...
  const completeMutation = useMutation({
    ...
    onSuccess: (data) => {
      localStorage.setItem(
        "fractionsResult",
        JSON.stringify({
          correct: userAnswers.filter((_, i) =>
      ...
      window.dispatchEvent(new Event("storage")); // Force navbar to refresh
    ...
    switch (question.kind) {
      ...
      case "equivalent":
        if (question.level === 0) {
          ...
          const answerArray = Array.isArray(question.answerSet)
            ? question.answerSet
            : Array.from(question.answerSet);
          ...
          const correct = Array.isArray(question.answerSet)
            ? question.answerSet
            : Array.from(question.answerSet);
          return (
  ...
  const handleSubmit = () => {
    ...
    if (
      currentQuestion.kind === "simplify" &&
      currentQuestion.gcd !== undefined &&
          ...
          <div className="space-y-6">
            <div className="text-center">
              <h3 className="text-lg font-semibold mb-4">
                What fraction does this bar represent?
              </h3>
              <FractionBar
              ...
              {gcdStep === 1 ? (
                <div className="space-y-4">
                  <p className="text-center">
                    What is the GCD of {currentQuestion.frac.num} and{" "}
                    {currentQuestion.frac.den}?
                  </p>
                  <div className="flex justify-center">
                    <Input
                      value={gcdAnswer}
                      ...
                      onKeyDown={(e) => {
                        if (
                          ...
                          if (
                            gcdAnswer.trim() === currentQuestion.gcd.toString()
                          ) {
                            setGcdStep(2);
                              ...
                              <div className="flex flex-col items-center justify-center min-w-[80px]">
                                {part.split("/")[0] === "?" ? (
                                  <>
                                    ...
                                    <span className="text-2xl font-bold h-10 flex items-center">
                                      {part.split("/")[1]}
                                    </span>
                                  </>
                                ) : (
            ...
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground">
                  Enter any equivalent fraction (multiply or divide both parts
                  by the same number)
                </p>
              </div>
            </div>
          ...
          return (
            <div className="space-y-6">
              <div className="text-center">
                ...
                <p className="text-sm text-muted-foreground mb-4">
                  Choose all fractions that equal the one above
                </p>
              </div>
                  ...
                  currentQuestion.given.includes(" ") ? (
                    ...
                    <div className="flex items-center space-x-2">
                      <span className="text-2xl font-bold">
                        {currentQuestion.given.split(" ")[0]}
                      </span>
                      <StackedFraction
                        numerator={
                          currentQuestion.given.split(" ")[1].split("/")[0]
                        }
                        denominator={
                          currentQuestion.given.split(" ")[1].split("/")[1]
                        }
                        className="text-xl"
                      />
                    </div>
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto py-6 px-4">
        <div className="mb-6 flex items-center justify-between">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            {FRACTIONS_PUZZLE_RULES.questionCount}
          </div>
        </div>
                      ...
                      {currentQuestion.kind === "equivalent" ? (
                        <div className="space-y-2">
                          <div className="font-medium">
                            {currentQuestion.frac.num}/
                            {currentQuestion.frac.den} is equivalent to:
                          </div>
                          <div className="space-y-1">
                            <div>
                              • {currentQuestion.frac.num * 2}/
                              {currentQuestion.frac.den * 2} (multiply by 2)
                            </div>
                            <div>
                              • {currentQuestion.frac.num * 3}/
                              {currentQuestion.frac.den * 3} (multiply by 3)
                            </div>
                          </div>
                          <div className="text-xs italic mt-1">
                            Tip: Multiply or divide both parts by the same
                            number!
                          </div>
                        </div>
                      ) : (
                ...
                {!showFeedback && (
                  <Button
                    ...
                    disabled={
                      ...
                      (currentQuestion.kind === "define" &&
                        (!numeratorInput || !denominatorInput)) ||
                      (currentQuestion.kind === "simplify" &&
                        (!numeratorInput || !denominatorInput)) ||
                      (currentQuestion.kind === "addSub" &&
                        (!numeratorInput || !denominatorInput)) ||
                      (currentQuestion.kind === "mulDiv" &&
                        (!numeratorInput || !denominatorInput)) ||
                      // Check for text inputs
                      (currentQuestion.kind === "equivalent" &&
                        currentQuestion.level <= 1 &&
                        !currentAnswer) ||
                      (currentQuestion.kind === "mixedImproper" &&
                        !currentAnswer) ||
                      ...
                      (currentQuestion.kind === "simplify" &&
                        ...
                        currentQuestion.level <= 1 &&
                        gcdStep === 2 &&
                        (!gcdAnswer || !numeratorInput || !denominatorInput))
                    }
                    size="lg"
                  >
                    Submit Answer
                  ...
                  <Card className="border-2 border-orange-200 shadow-xl">
                    ...
                    <CardContent className="space-y-4">
                      <p className="text-gray-700">
                        If you leave now, you'll lose your progress in this
                        skill and need to start over.
                      </p>
